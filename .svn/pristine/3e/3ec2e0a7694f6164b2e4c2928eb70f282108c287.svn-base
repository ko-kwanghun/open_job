...
class AddValidationRuleBehavior extends ModelBehavior {
	...
	*/
	...
	function phone_no_max( &$model, $wordvalue , $no2_filed ,$no3_filed,  $min=10,$max=11 ,$empty=true){
		$field = current($wordvalue);
		$field2 =  isset($model->data[$model->alias][$no2_filed ]) ? $model->data[$model->alias][$no2_filed ] : null;
		$field3 =  isset($model->data[$model->alias][$no3_filed ]) ? $model->data[$model->alias][$no3_filed ] : null;

		$length = mb_strlen( $field )+mb_strlen( $field2 )+mb_strlen( $field3 );
		if($length==0&& $empty){
			return true;
		}
	...
	function post_codes( &$model, $wordvalue , $no2_filed){

		$field = current($wordvalue);
		$field2 =  isset($model->data[$model->alias][$no2_filed ]) ? $model->data[$model->alias][$no2_filed ] : null;

		$length = mb_strlen( $field );
		$length2 = mb_strlen( $field2 );
		if($length==0){
			return true;
		}elseif($length2!=null){
			return true;
